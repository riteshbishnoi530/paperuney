...
function Header() {
  ...
  return (
    <div className='flex justify-between items-center max-w-[1332px] mx-auto px-4'>
      <Link className='bg-[url("/assets/images/btn-bg.webp")] bg-center bg-cover bg-no-repeat font-luckiest text-white text-[28px] max-lg:text-xl max-sm:text-base max-sm:py-[10px] py-4 max-lg:py-3 max-lg:px-4 px-6' href="/">BUY $PEPERUNEY
      </Link>
      <div className='flex gap-10 max-sm:gap-3 items-center'>
        <button onClick={() => setOpen(!open)} className="flex cursor-pointer items-center gap-[14px] max-sm:gap-1 relative">
          <Image src={'/assets/images/flag.webp'} alt="flag" height={31} width={41} className="max-sm:h-6 max-sm:w-8 pointer-events-none" />
          <div className={`absolute flex flex-col gap-2 transition-all duration-200 ${open ? "opacit-100 top-14 max-sm:top-10" : "opacity-0 top-0"}`}>
            <Image src={'/assets/images/india-flag.webp'} alt="indian-flag" height={31} width={41} className="max-sm:h-6 max-sm:w-8 pointer-events-none" />
            <Image src={'/assets/images/flag.webp'} alt="flag" height={31} width={41} className="max-sm:h-6 max-sm:w-8 pointer-events-none" />
          </div>
